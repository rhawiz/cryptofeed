...
class KuCoin(Feed):
    ...
    async def _process_l2_book(self, msg: dict, symbol: str, timestamp: float):
        ...
        await self.book_callback(book_type=L2_BOOK, book=self._l2_book[symbol], timestamp=timestamp, receipt_timestamp=time.time(), delta=delta, raw=msg, sequence_number=data['sequenceEnd'])
