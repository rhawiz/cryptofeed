...
class KuCoinFutures(Feed):
    ...
    async def _process_l2_book(self, msg: dict, symbol: str, timestamp: float):
        ...
        #             delta[side].append((price, amount))
        #
        # await self.book_callback(L2_BOOK, self._l2_book[symbol], timestamp, delta=delta, raw=msg,
        #                          sequence_number=data['sequenceEnd'])
        #

        delta = {BID: [], ASK: []}
        # for s, side in (('bids', BID), ('asks', ASK)):
        # for update in data['change']:
        update = data['change']
        parts = update.split(",")
        price = Decimal(parts[0])
        side = ASK if parts[1] == "sell" else BID
